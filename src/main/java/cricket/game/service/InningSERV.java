...
public class InningSERV {
	...
	public ScoreCardData inning() {
		...
		if (scoreCardData.isLegal()) {
			...
			if (scoreCardData.getInningType() == 2) {
				//Minus Remaining balls for 2nd inning to display remaining balls 
				scoreCardData.setRemainingBall(scoreCardData.getTotalOvers() * 6 - (1));
				

//				scoreCardData.setRemainingOvers(overcountSERV.remainingOvers(scoreCardData.getRemainingBall()));
			}
			
			//this will add into database for the following condition in FinalScoreCard for team one and two as per condition
			if (scoreCardData.getRandomRun() == 0 || scoreCardData.getBallCount() == scoreCardData.getTotalOvers() * 6) {
